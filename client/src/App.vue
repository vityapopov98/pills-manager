...
<button class="btnMode" @click="changeMode()">{{btnModeText}}</button>
<UserPills v-if="mode == 'pills'" />
<Recipe v-if="mode == 'recipe'" />
...
export default {
  ...
  methods:{
    changeMode(){
      if(this.mode == 'pills') {
        this.mode = 'recipe'
        this.btnModeText = '<- Вернуться в аптечку'
      }
      else {
        this.mode = 'pills'
        this.btnModeText = 'Добавить рецепт ->'
      }
    }
    // addPills(){
    //   var data = {
